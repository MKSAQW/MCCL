...
class DeepLabV3Plus(nn.Module):
    ...
    def forward(self, x, only_need_decoder=False ,need_pre_logit=False,need_fp=False):
        if only_need_decoder:
        ...
        if need_fp:
            feature,outs = self._decode(torch.cat((c1, nn.Dropout2d(0.5)(c1))),
                                torch.cat((c4, nn.Dropout2d(0.5)(c4))))
            outs = F.interpolate(outs, size=(h, w), mode="bilinear", align_corners=True)
            out, out_fp = outs.chunk(2)

            return feature,out, out_fp

        feature,out = self._decode(c1, c4)
        pre_logit = out.clone()
        out = F.interpolate(out, size=(h, w), mode="bilinear", align_corners=True)
        # return out
        if need_pre_logit:
            return feature, out, pre_logit
        else:
            return feature,out
    ...
    def decoder_noise(self,feature_noise,h,w):
        out = self.classifier(feature_noise)
        out = F.interpolate(out, size=(h, w), mode="bilinear", align_corners=True)
        return out
